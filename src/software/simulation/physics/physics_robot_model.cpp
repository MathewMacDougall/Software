...
b2PolygonShape* PhysicsRobotModel::getMainRobotBodyShape(double mouth_depth)
{
    ...
    double y =
        std::sqrt(std::pow(ROBOT_MAX_RADIUS_METERS, 2) -
                  std::pow(DIST_TO_FRONT_OF_ROBOT_METERS - mouth_depth, 2));

    Vector starting_vector(DIST_TO_FRONT_OF_ROBOT_METERS - mouth_depth, y);
...
b2PolygonShape* PhysicsRobotModel::getRobotBodyShapeFrontLeft(double mouth_depth)
{
    ...
    // order)
    auto shape_points = getRobotFrontLeftShapePoints(mouth_depth);
...
b2PolygonShape* PhysicsRobotModel::getRobotBodyShapeFrontRight(double mouth_depth)
{
    auto shape_points = getRobotFrontLeftShapePoints(mouth_depth);
...
std::vector<Point> PhysicsRobotModel::getRobotFrontLeftShapePoints(double mouth_depth)
{
    // Assuming the robot is at (0, 0) and facing the +x axis (aka has an orientation of
    // 0) First find the y-coordinate of the front-left edge of the body by solving x^2 +
    // y^2 = ROBOT_RADIUS^2
    ...
    std::vector<Point> vertices{
        Point(DIST_TO_FRONT_OF_ROBOT_METERS - mouth_depth, y),
        Point(DIST_TO_FRONT_OF_ROBOT_METERS - mouth_depth, DRIBBLER_WIDTH_METERS / 2.0),
