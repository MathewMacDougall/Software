...
// this to calculate the depth.
const double PhysicsRobot::mouth_depth =
...
void PhysicsRobot::setupRobotBodyFixtures(const RobotState&,
                                          const double mass_kg)
{
    b2PolygonShape* main_body_shape =
        PhysicsRobotModel::getMainRobotBodyShape(mouth_depth);
    b2PolygonShape* front_left_body_shape =
        PhysicsRobotModel::getRobotBodyShapeFrontLeft(mouth_depth);
    b2PolygonShape* front_right_body_shape =
        PhysicsRobotModel::getRobotBodyShapeFrontRight(mouth_depth);
...
void PhysicsRobot::setupMouthFixture(const RobotState &robot_state) {
    b2FixtureDef robot_mouth_fixture_def;
    robot_mouth_fixture_def.density = ROBOT_MOUTH_DENSITY;
    // We explicitly choose to make the mouth NOT a sensor, because sensor fixtures do
    // not trigger PreSolve contact callbacks, which we rely on to apply dribbling force
    // at every physics step
    robot_mouth_fixture_def.isSensor = false;
    robot_mouth_fixture_def.restitution = ROBOT_MOUTH_RESTITUTION;
    robot_mouth_fixture_def.friction = ROBOT_MOUTH_FRICTION;
    robot_mouth_fixture_def.userData =
            new PhysicsObjectUserData({PhysicsObjectType::ROBOT_MOUTH, this});
    ...
    b2Vec2 mouth_shape_vertices[num_vertices] = {
            createVec2(Point(DIST_TO_FRONT_OF_ROBOT_METERS, DRIBBLER_WIDTH_METERS / 2.0)),
            createVec2(Point(DIST_TO_FRONT_OF_ROBOT_METERS - mouth_depth,
                             DRIBBLER_WIDTH_METERS / 2.0)),
            createVec2(Point(DIST_TO_FRONT_OF_ROBOT_METERS - mouth_depth,
                             -DRIBBLER_WIDTH_METERS / 2.0)),
            createVec2(Point(DIST_TO_FRONT_OF_ROBOT_METERS, -DRIBBLER_WIDTH_METERS / 2.0))};
    b2PolygonShape* mouth_shape = new b2PolygonShape();
    mouth_shape->Set(mouth_shape_vertices, num_vertices);
    robot_mouth_fixture_def.shape = mouth_shape;
    robot_body->CreateFixture(&robot_mouth_fixture_def);
    delete mouth_shape;
}
...
void PhysicsRobot::registerMouthBallContactCallback(
    std::function<void(PhysicsRobot*, PhysicsBall*)> callback)
{
    mouth_ball_contact_callbacks.emplace_back(callback);
}

void PhysicsRobot::registerMouthBallStartContactCallback(
    std::function<void(PhysicsRobot*, PhysicsBall*)> callback)
{
    mouth_ball_start_contact_callbacks.emplace_back(callback);
}

void PhysicsRobot::registerMouthBallEndContactCallback(
    std::function<void(PhysicsRobot*, PhysicsBall*)> callback)
{
    mouth_ball_end_contact_callbacks.emplace_back(callback);
}

std::vector<std::function<void(PhysicsRobot*, PhysicsBall*)>>
PhysicsRobot::getMouthBallContactCallbacks() const
{
    return mouth_ball_contact_callbacks;
}

std::vector<std::function<void(PhysicsRobot*, PhysicsBall*)>>
PhysicsRobot::getMouthBallStartContactCallbacks() const
{
    return mouth_ball_start_contact_callbacks;
}

std::vector<std::function<void(PhysicsRobot*, PhysicsBall*)>>
PhysicsRobot::getMouthBallEndContactCallbacks() const
{
    return mouth_ball_end_contact_callbacks;
}
